/* ============================================================================
   lets-scroll — pure video scroll-scrubbed camera-flight engine
   ========================================================================== */
...
function mountLetsScroll(container, config) {
  ...
  // Build segments directly from sections
  const SEGMENTS = [];
  ...
  // Mount Direct Video Scenes (Zero image dependency)
  SEGMENTS.forEach((s, idx) => {
    ...
    // Initial scene 0 visible
    if (idx === 0) {
      scene.style.opacity = '1';
      scene.style.zIndex = '120';
    } else {
  ...
  function jumpTo(i) {
    ...
    window.scrollTo({ top: seg.start + (seg.end - seg.start) * 0.5, behavior: 'smooth' });
  }
  ...
  function read() {
    ...
    const near = clamp((((y - cur.start) / (cur.end - cur.start)) > 0.5 ? cur.si + 1 : cur.si), 0, N - 1);
    if (near !== activeIndex) {
      activeIndex = near;
  ...
  function raf() {
    for (let i = 0; i < NSEG; i++) {
      ...
      const dur = s.video.duration || 10;
      const t = clamp(s.cur, 0, 0.999) * dur;

      if (s.visible && Math.abs(s.video.currentTime - t) > 0.015) {
        try {
  ...
  // Wake up decoders on first user interaction
  let userReady = false;
    ...
    SEGMENTS.forEach(s => {
      if (s.video) {
        ...
        try {
          ...
          if (p && p.then) {
            p.then(() => { try { s.video.pause(); } catch (e) {} }).catch(() => {});
          }
        } catch (e) {}
      }
    });
  ...
  window.addEventListener('pointerdown', primeAllVideos, { once: true, passive: true });
  window.addEventListener('touchstart', primeAllVideos, { once: true, passive: true });
  window.addEventListener('wheel', primeAllVideos, { once: true, passive: true });
  window.addEventListener('scroll', primeAllVideos, { once: true, passive: true });
...
function injectCSS() {
  ...
  .sw-scene{position:absolute;inset:0;opacity:0;overflow:hidden;will-change:opacity;transition:opacity 0.2s ease;}
  .sw-scene__video{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:center 42%;display:block;}
  ...
  @media (max-width:860px){
    ...
    .sw-scene__video{object-position:center 46%;}
